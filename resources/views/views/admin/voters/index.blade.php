...
@section('content')
    <div class="px-8 sm:px-0">
        ...
        <form action="{{ route('admin.voters.update') }}" method="POST" class="my-4 flex items-end w-64">
            @csrf
            <label class="input-label inline-flex my-0 flex-1" for="amount">
                Amount of tokens
                <input id="amount" type="tel" required minlength="1" maxlength="3" size="5" name="amount" class="input rounded-r-none" placeholder="12"/>
            </label>
